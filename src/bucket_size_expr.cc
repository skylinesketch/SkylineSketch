...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        cout << "usage: ./bucket_size_expr bucket_slotNum attribute_num dataset" << endl;
        return 0;
    }
    ...
    int bucket_slotNum = atoi(argv[1]);
    ...
    int memory = 10;
    int bucket_num = std::ceil((double)(memory * 1024 - global_bucket_slotNum * slot_size -
    ...
    cout << "Bucket_slotNum: " << bucket_slotNum << endl;
    ...
    chrono::duration<double> query_span =
    chrono::duration_cast<chrono::duration<double>>(query_end-query_start);
